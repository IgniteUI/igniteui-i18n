import type { IPaginatorResourceStrings } from 'igniteui-i18n-core';

/**
 * Bulgarian resource strings for Ignite UI Paginator
 */
export const PaginatorResourceStringsBG: IPaginatorResourceStrings = {
  paginator_label: 'Елементи на страница',
  paginator_pager_text: 'от',
  paginator_first_page_button_text: 'Към първата страница',
  paginator_previous_page_button_text: 'Предишна страница',
  paginator_last_page_button_text: 'Към последната страница',
  paginator_next_page_button_text: 'Следваща страница',
} satisfies Required<IPaginatorResourceStrings>;

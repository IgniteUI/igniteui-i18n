...
describe('number formatting', () => {
    ...
    it('should format numbers with provided currency settings for different currencies', () => {
        expect(numberFormatter.formatNumber(12345, 'en-GB', { style: "currency", currency: "GBP" })).equal('£12,345.00');
        expect(numberFormatter.formatNumber(12345, 'bg', { style: "currency", currency: "BGN" })).equal('12 345,00 лв.');
        expect(numberFormatter.formatNumber(12345, 'ja', { style: "currency", currency: "JPY" })).equal('￥12,345');
        expect(numberFormatter.formatNumber(12345, 'es', { style: "currency", currency: "EUR" })).equal('12.345,00 €');
        expect(numberFormatter.formatNumber(12345, 'pl', { style: "currency", currency: "PLN" })).equal('12 345,00 zł');
    })
